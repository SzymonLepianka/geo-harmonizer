...
export function MapView({ projectId, layers, runs }: { projectId: string; layers: Layer[]; runs: AnalysisRun[] }) {
  ...
  return <div className="map-layout">
    <aside className="map-sidebar">
      ...
      <label>Wyniki analizy<select value={selectedRun} onChange={event => setSelectedRun(event.target.value)}><option value="">Bez wyników</option>{runs.filter(run => run.status === 'DONE').map(run => <option key={run.id} value={run.id}>{run.name}</option>)}</select></label>
      <div className="legend"><h4>Priorytet wyników</h4><span className="dot info"/> INFO <span className="dot low"/> LOW <span className="dot medium"/> MEDIUM <span className="dot high"/> HIGH</div>
    </aside>
    <div className="map-canvas" ref={target}/>
